...
async function extractEpisodes(url) {
    ...
    try {
        ...
        episodes = await getEpisodesWithLanguageSubs(`${ BASE_URL }${ episodesList[0] }?origin=${ origin }`, 'Portuguese', 'vtt');
        if(episodes.length <= 0) throw new Error('No episodes with Portuguese subtitles found.');
...
async function extractStreamUrl(url) {
    try {
        ...
        const subtitles = json?.props?.pageProps?.episode?.subData.find(sub => sub.type === 'vtt' && sub.label === 'Portuguese');
        if(subtitles == null) throw new Error('No Portuguese subtitles found');
