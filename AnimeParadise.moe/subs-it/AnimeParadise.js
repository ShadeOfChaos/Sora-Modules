...
async function searchResults(keyword) {
    ...
    try {
        ...
        const html = await response.text();//REVERT
...
async function extractEpisodes(url) {
    ...
    try {
        ...
        episodes = await getEpisodesWithLanguageSubs(`${ BASE_URL }${ episodesList[0] }?origin=${ origin }`, 'Italian', 'vtt');
        if(episodes.length <= 0) throw('No episodes with Italian subtitles found.');
...
async function extractStreamUrl(url) {
    try {
        ...
        const subtitles = json?.props?.pageProps?.episode?.subData.find(sub => sub.type === 'vtt' && sub.label === 'Italian');
        if(subtitles == null) throw('No Italian subtitles found');
